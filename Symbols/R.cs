...
{
    public class R : Symbol
    {
        public R(Entity Src, List<PortS> P)
            : base(Src, P)
        {
            ...
            this.boxX = 1;
            this.boxY = 1;
            foreach (string Po in this.Src.Ports) if (Po == "0") this.Zero = true; //Проверяем, является ли однопортовым элементом
        }
            ...
            : base(Src)
        {
            ...
            this.boxX = 1;
            this.boxY = 1;
            foreach (string P in this.Src.Ports) if (P == "0") this.Zero = true; //Проверяем, является ли однопортовым элементом
        }
        ...
        public override void DrawMe(Graphics e, int kx, int ky, float x0, float y0) //Масштаб + начальные коо
        {
            ...
            e.DrawString("R", Setting.Global.Font, Setting.Global.Brush, Setting.Global.Rec, Setting.Global.Format); //Зашибись, сама центрует
        }
        ...
        public override void FillPortPoint(Symbol S, PortS P)
        {
            ...
            {
                P.AddPoint(new Point(this.X() + width / 2, this.Y()));
            }
        }
